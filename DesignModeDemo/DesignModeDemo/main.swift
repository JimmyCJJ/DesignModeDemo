...
// 优点：
// 1.扩展性好。能够在不修改对象结构中元素的情况下，为对象结构中的元素添加新功能。
// 2.复用性好。可以通过访问者来定义整个对象结构通用的功能，从而提高系统的复用功能。
// 3.灵活性好。访问者模式将数据结构与作用于结构上的操作解耦，使得操作集合可相对自由。
// 缺点：
// 1.增加新的数据结构很困难。在访问者模式中，每增加一个新的元素类，都要在每一个具体访问者类中增加响应的具体操作，违背了 “开闭原则”。
// 2.破坏封装。访问者模式中具体元素对访问者公布细节，这破坏了对象的封装性。
// 3.违反了依赖倒置原则。访问模式依赖了具体类，而没有依赖抽象类。
// 4.具体元素对访问者公布细节。也就是说访问者关注了其他类的内部细节，这是迪米特法则所不建议的，这样造成了具体元素变得更比较困难。
// 适用场景：
...
func visitorMode() {
    let phone = Phone()
    phone.printPhoneSoftWareInfo()
    let updateV1 = PhoneUpdateV1()
    phone.accept(updateV1)
    phone.printPhoneSoftWareInfo()
}
